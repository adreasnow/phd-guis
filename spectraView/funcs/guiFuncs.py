from PyQt6.QtWidgets import QSpinBox, QDoubleSpinBox, QListWidget, QListWidgetItem, QComboBox
from superqt import QRangeSlider

def add_items_list(widget: QListWidget, items: list[object]):
    widget.clear()
    for i in items:
...
def add_items_list_dict(widget: QListWidget, items: dict[object]):
    widget.clear()
    for i in items:
        item_to_add = QListWidgetItem()
...
def add_items_combo(widget: QComboBox, items: list[object]):
    widget.clear()
    for i in items:
        widget.addItem(str(i), userData=i)

def add_items_combo_dict(widget: QComboBox, items: dict[object]):
    widget.clear()
    for i in items:
